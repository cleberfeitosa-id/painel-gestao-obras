-- Corrige o escopo e o preco efetivo usado nos resumos de medicao.
-- O catalogo continua sendo a fonte primaria; itens de orcamento vinculados
-- servem como fallback para catalogos legados com valor_unitario igual a zero.

do $$
begin
  if exists (
    select 1
    from public.tarefa_medicoes
...
declare
  v_obra_id uuid;
  v_quantidade_ids integer;
  v_quantidade_validos integer;
begin
  select m.obra_id into v_obra_id
  ...
  if v_obra_id is null then
    raise exception 'Item do catalogo nao pertence a medicao';
  end if;

  select count(*) into v_quantidade_ids
  from (select distinct unnest(coalesce(p_orcamento_item_ids, '{}'::uuid[])) as id) ids;
  select count(*) into v_quantidade_validos
  from public.orcamento_itens oi
  join public.orcamentos o on o.id = oi.orcamento_id
  where oi.id = any(coalesce(p_orcamento_item_ids, '{}'::uuid[]))
    and o.obra_id = v_obra_id;
  if v_quantidade_ids <> v_quantidade_validos then
    raise exception 'Existem itens do orcamento invalidos para esta obra';
  end if;

  update public.catalogo_precos
  set nome = p_nome, valor_unitario = p_valor_unitario, unidade = p_unidade
...
begin
  ...
  select count(*) into v_quantidade_ids from (select distinct unnest(coalesce(p_orcamento_item_ids, '{}'::uuid[])) id) ids;
  select count(*) into v_quantidade_validos
  ...
  if v_quantidade_ids <> v_quantidade_validos then raise exception 'Itens do orcamento invalidos'; end if;
  insert into public.catalogo_precos (medicao_id, nome, valor_unitario, unidade, criado_por)
  values (p_medicao_id, p_nome, p_valor_unitario, p_unidade, p_criado_por)
  returning id into v_catalogo_id;
  insert into public.catalogo_precos_orcamento_itens (catalogo_id, orcamento_item_id)
  select v_catalogo_id, ids.id from (select distinct unnest(coalesce(p_orcamento_item_ids, '{}'::uuid[])) id) ids;
...
-- Mantem os paineis financeiros alinhados ao mesmo preco efetivo da medicao.
-- Esta funcao precisa ser criada antes de qualquer RPC que a referencie.
...
language sql stable set search_path = '' as $$
  ...
  join public.orcamentos o on o.id = oi.orcamento_id
  left join (
    select j.orcamento_item_id, sum(tm.quantidade) quantidade_medida,
      sum(tm.quantidade * public.preco_efetivo_catalogo(c.id)) medido,
      sum(tm.quantidade) filter (where t.status = 'concluido') quantidade_executada,
      sum(tm.quantidade * public.preco_efetivo_catalogo(c.id)) filter (where t.status = 'concluido') executado
    from public.catalogo_precos_orcamento_itens j
